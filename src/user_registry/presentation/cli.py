...
def create(
    ...
    password: str = typer.Option(
        None, "--password", "-p", help="Senha (se não informar, será perguntada)"
    ),
):
    ...
    try:
        ...
        console.print(
            Panel.fit(
                "✅ Usuário criado com sucesso!", title="create", border_style="green"
            )
        )
...
def show(email: str = typer.Option(..., "--email", "-e", help="Email do usuário")):
    ...
    if not user:
        console.print(
            Panel.fit("❌ Usuário não encontrado.", title="show", border_style="red")
        )
        raise typer.Exit(code=1)
...
def update(
    ...
    try:
        ...
        console.print(
            Panel.fit("✅ Nome atualizado!", title="update", border_style="green")
        )
    except Exception as ex:
...
def deactivate(
    ...
    try:
        ...
        console.print(
            Panel.fit(
                "✅ Usuário desativado.", title="deactivate", border_style="yellow"
            )
        )
    except Exception as ex:
        console.print(
            Panel.fit(f"❌ Erro: {ex}", title="deactivate", border_style="red")
        )
        raise typer.Exit(code=1)
...
def activate(email: str = typer.Option(..., "--email", "-e", help="Email do usuário")):
    ...
    try:
        ...
        console.print(
            Panel.fit("✅ Usuário ativado.", title="activate", border_style="green")
        )
    except Exception as ex:
